...
// Fancy Chems are too much of a headache to work out right now
...
/atom/proc/add_milk_floor(mob/living/carbon/M)
	if( istype(src, /turf/simulated) )
		...
		if(M.reagents)
			M.reagents.trans_to(this, M.reagents.total_volume / 10)
	/*
/mob/living/carbon/proc/ejaculate(var/stun = 1, var/distance = 0, var/message = 1)
