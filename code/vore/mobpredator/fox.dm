/mob/living/simple_animal/pet/fox
	...
	turns_per_move = 5
	see_in_dark = 6
	butcher_results = /obj/item/weapon/reagent_containers/food/snacks/meat/fox
...
/mob/living/simple_animal/pet/fox/Life()
		...
		if(!stat && !resting && !buckled)
			for(var/mob/living/simple_animal/mouse/M in loc)
				if(isPredator) //If the fox is a predator,
					...
					emote("me", 1, "greedily stuffs [M] into their gaping maw!")
					sleep(30)
					emote("me", 1, "swallows down [M] into their hungry gut!")
					...
					if(!M.stat)
						M.splat()
						emote("me", 1, pick("bites \the [M]!","toys with \the [M].","chomps on \the [M]!"))
						movement_target = null
	...
	for(var/mob/living/simple_animal/mouse/snack in oview(src,5))
		if(snack.stat < DEAD && prob(15))
			emote("me", 1, pick("hunkers down!","acts stealthy!","eyes [snack] hungrily."))
		break
	...
	if(!stat && !resting && !buckled) //SEE A MICRO AND ARE A PREDATOR, EAT IT!
		for(var/mob/living/carbon/human/food in oview(src, 5))
			...
			if(food.playerscale <= RESIZE_A_SMALLTINY)
				if(prob(10))
					emote("me", 1, pick("eyes [food] hungrily!","licks their lips and turns towards [food] a little!","pants as they imagine [food] being in their belly."))
					break
				else
					if(prob(5))
		...
		for(var/mob/living/carbon/human/bellyfiller in oview(1, src))
			...
			if(bellyfiller.playerscale <= RESIZE_A_SMALLTINY && isPredator)
				movement_target = null
				emote("me", 1, pick("slurps [bellyfiller] with their slimey tongue.","looms over [bellyfiller] with their maw agape.","sniffs at [bellyfiller], their belly grumbling hungrily."))
				sleep(10)
				emote("me", 1, "starts to scoop [bellyfiller] into their maw!")
				sleep(swallowTime)
				if(bellyfiller in oview(1, src))
					emote("me", 1, "swallows down [bellyfiller] with a happy yap!")
					src.insides.nom_mob(bellyfiller)
					log_attack("[key_name(bellyfiller)] got eaten by [src]!")
					playsound(src, 'sound/vore/gulp.ogg', 100, 1)
				else
...
// /mob/living/simple_animal/pet/fox/hitby(atom/movable/AM)
//	. = ..()
//	set_flee_target(M.thrower? M.thrower : src.loc)

/mob/living/simple_animal/pet/fox/MouseDrop(atom/over_object)
...
// /mob/living/simple_animal/pet/fox/get_scooped(var/mob/living/carbon/grabber)
//	if (stat >= DEAD)
//		return //since the holder icon looks like a living cat
//	..()
/*
// No idea how to impliment this right now - Poojawa
//Basic friend AI
/mob/living/simple_animal/pet/fox/fluff
	var/mob/living/carbon/human/friend
	var/befriend_job = null

/mob/living/simple_animal/pet/fox/fluff/handle_movement_target()
	if (friend)
		var/follow_dist = 5
...
/mob/living/simple_animal/pet/fox/fluff/Life()
	..()
	if (stat || !friend)
		return
...
/mob/living/simple_animal/pet/fox/fluff/verb/friend()
...
/mob/living/simple_animal/pet/fox/stomach_emotes = list(
...
/mob/living/simple_animal/pet/fox/stomach_emotes_d = list(
...
/mob/living/simple_animal/pet/fox/fluff/Renault/stomach_emotes = list(
...
/mob/living/simple_animal/pet/fox/fluff/Renault/stomach_emotes_d = list(
