...
NanoStateManager = function () 
{
	...
	var init = function () 
	{
		...
		VoreStateManager.setCurrentState(stateKey);
	...
	return {
		...
		addBeforeUpdateCallbacks: function (callbacks) {		
			for (var callbackKey in callbacks) {
				...
				VoreStateManager.addBeforeUpdateCallback(callbackKey, callbacks[callbackKey]);
			}
		},
		...
		addAfterUpdateCallbacks: function (callbacks) {		
			for (var callbackKey in callbacks) {
				...
				VoreStateManager.addAfterUpdateCallback(callbackKey, callbacks[callbackKey]);
			}
		},
		...
		{
			if (!(state instanceof VoreStateClass))
			{
				alert('ERROR: Attempted to add a state which is not instanceof VoreStateClass');
				return;
			}
			if (!state.key)
			{
