        ...
        for (var key in _templateData)
        {
            ...
            $.when($.ajax({
                ...
                .done(function(templateMarkup) {
                    ...
                    try
                    {
                        VoreTemplate.addTemplate(key, templateMarkup);
                    }
                    catch(error)
                    {
    ...
    return {
		...
		addHelper: function (helperName, helperFunction) {
			if (!jQuery.isFunction(helperFunction)) {
				alert('VoreTemplate.addHelper failed to add ' + helperName + ' as it is not a function.');
				return;	
			}
		...
		addHelpers: function (helpers) {		
			for (var helperName in helpers) {
				...
				VoreTemplate.addHelper(helperName, helpers[helperName]);
			}
		},
