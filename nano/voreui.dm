...
voreui class (or whatever Byond calls classes)

voreui is used to open and update nano browser uis
**********************************************************/
...
/datum/voreui
  ...
  *
  * @return /voreui new voreui object
  */
/datum/voreui/New(nuser, nsrc_object, nui_vkey, ntemplate_filename, ntitle = 0, nwidth = 0, nheight = 0, var/atom/nref = null, var/datum/voreui/master_ui = null, var/datum/topic_state/custom_state = null)
	user = nuser
	src_object = nsrc_object
	ui_vkey = nui_vkey
	window_id = "[ui_vkey]\ref[src_object]"
...
//	src.custom_state = custom_state ? custom_state : new/datum/topic_state()
  ...
  * @return nothing
  */
/datum/voreui/proc/add_common_assets()
  ...
  * @return nothing
  */
/datum/voreui/proc/set_status(state, push_update)
	if (state != status) // Only update if it is different
		if (status == VSTATUS_DISABLED)
			status = state
			if (push_update)
				update()
		else
  ...
  * @return nothing
  */
/datum/voreui/proc/update_status(var/push_update = 0)
	var/atom/movable/host = src_object.vore_host()
	var/new_status = host.CanUseTopic(user, list()) //, custom_state)
	if(master_ui)
		new_status = min(new_status, master_ui.status)
	if(new_status == VSTATUS_CLOSE)
		vclose()
	else
  ...
  * @return nothing
  */
/datum/voreui/proc/set_auto_update(nstate = 1)
  ...
  * @return nothing
  */
/datum/voreui/proc/set_initial_data(list/data)
	initial_data = data
  ...
  */
/datum/voreui/proc/get_config_data()
  ...
  */
/datum/voreui/proc/get_send_data(var/list/data)
  ...
  * @return nothing
  */
/datum/voreui/proc/set_window_options(nwindow_options)
  ...
  * @return nothing
  */
/datum/voreui/proc/add_stylesheet(file)
  ...
  * @return nothing
  */
/datum/voreui/proc/add_script(file)
	scripts.Add(file)
  ...
  * @return nothing
  */
/datum/voreui/proc/add_template(key, filename)
  ...
  * @return nothing
  */
/datum/voreui/proc/set_layout_key(nlayout_key)
  ...
  * @return nothing
  */
/datum/voreui/proc/set_auto_update_layout(nstate)
  ...
  * @return nothing
  */
/datum/voreui/proc/set_auto_update_content(nstate)
  ...
  * @return nothing
  */
/datum/voreui/proc/set_state_key(nstate_key)
  ...
  * @return nothing
  */
/datum/voreui/proc/set_show_map(nstate)
	show_map = nstate
  ...
  * @return nothing
  */
/datum/voreui/proc/set_map_z_level(nz)
	map_z_level = nz
  ...
  * @return nothing
  */
/datum/voreui/proc/use_on_close_logic(state)
...
//	vopened()
  ...
  * @return nothing
  */
/datum/voreui/proc/vclose()
	is_auto_updating = 0
//	vclosed()
	user << browse(null, "window=[window_id]")
	for(var/datum/voreui/child in children)
		child.vclose()
  ...
  * @return nothing
  */
/datum/voreui/proc/on_close_winset()
	if(!user.client)
		return
	var/params = "\ref[src]"
 ...
 * @return nothing
 */
/datum/voreui/proc/load_cached_data(var/data)
	cached_data = data
	return
  ...
  * @return nothing
  */
/datum/voreui/proc/push_data(data, force_push = 0)
	update_status(0)
	if (status == VSTATUS_DISABLED && !force_push)
  ...
  * If the UI status is currently VSTATUS_INTERACTIVE then call the src_object Topic()
  * If the src_object Topic() returns 1 (true) then update all UIs attached to src_object
  *
  * @return nothing
  */
/datum/voreui/Topic(href, href_list)
	update_status(0) // update the status
	if (status != VSTATUS_INTERACTIVE || user != usr) // If UI is not interactive or usr calling Topic is not the UI user
		return
/*
	...
	if ((src_object && src_object.Topic(href, href_list, 0)) || map_update)
  ...
  * @return nothing
  */
/datum/voreui/proc/process_vui(update = 0)
	if (!src_object || !user)
		vclose()
		return
  ...
  * @return nothing
  */
/datum/voreui/proc/update(var/force_open = 0)
	src_object.ui_interact(user, ui_vkey, src, force_open, master_ui) //, custom_state)